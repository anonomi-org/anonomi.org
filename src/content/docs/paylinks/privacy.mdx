...
Donating happens over Tor only, so donation requests never traverse an exit node — the onion service is reached end-to-end inside the network. Your ISP can see that you are using Tor, but not that you visited Anonomi or that you donated.

Donors should:
- Use Tor Browser
- Verify the onion address against one published in this documentation
...
The Paylinks API is reachable only as an onion service:

- **Onion**: `http://b7o4bzmc5ylx3ynbg4pvxs4vwifviuzkonle66uzdrv5ff7vj5pln7yd.onion`

It accepts requests only from the onion website's origin. There is no clearnet endpoint — the previous one has been decommissioned. See [Why Paylinks runs only on Tor](/docs/paylinks/tor-separation).
...
- [Infrastructure & Data Security](/docs/paylinks/infrastructure)
