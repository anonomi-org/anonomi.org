...
Anonomi Messenger is distributed through F-Droid and direct download, never through Google Play.

If you install from F-Droid, the client verifies each download against F-Droid's signed index
before installing. If you download an APK directly, **verification is your responsibility**.
...
- **F-Droid** — https://f-droid.org/packages/org.anonomi.android/
- **GitHub Releases** — https://github.com/anonomi-org/anonomi-android/releases

Both channels ship APKs signed with the same key. See [Downloads](/docs/downloads) for details.
...
The two channels build different APKs, so **their checksums differ**. Always compare against
the checksum for the channel you downloaded from. A mismatch across channels is expected.
